...
function fallbackIdealBand(config) {
  const ideal = (num(config.targetMin, 70) + num(config.targetMax, 140)) / 2;
  const margin = Math.max(5, num(config.idealTargetMargin, 10));
  return { ideal, low: Math.max(num(config.hypoThreshold, 70), ideal - margin), high: ideal + margin };
}
...
function renderSvgChart(simulation) {
  ...
  const band = model.idealBand || fallbackIdealBand(model.config || {});
  ...
  const rawMax = Math.max(band.high + 20, Math.max(...allGlucoseValues) + 18);
  ...
  return `
    <div class="glucosa-chart-card">
      <svg class="glucosa-fusion-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Curva GlucosaTrack con rango ideal">
        ...
        <rect x="${pad.left}" y="${y(band.high)}" width="${plotW}" height="${Math.max(0, y(band.low) - y(band.high))}" rx="10" fill="#D0F0EA" opacity="0.72"></rect>
        ...
        <text x="${width - pad.right}" y="${(y(band.low) - 5).toFixed(1)}" text-anchor="end" font-size="11" fill="#0F5A4D">ideal ±${fmt((band.high - band.low) / 2)} mg/dL</text>
      </svg>
...
function renderSafetyNote(plan, manualSafety) {
  const band = plan.idealBand || plan.safety?.band;
  const parts = [];
  if (band) parts.push(`Objetivo usado: ${fmt(band.ideal)} mg/dL ±${fmt((band.high - band.low) / 2)}.`);
  if (plan.strategyAdjusted) parts.push(`Comida lenta detectada: se cambió ${plan.requestedStrategy} → ${plan.strategy}.`);
  if (plan.doseScale && plan.doseScale !== 1) parts.push(`Dosis reducida al ${fmt(plan.doseScale * 100)}% para evitar bajadas.`);
  if (manualSafety?.hasHypoRisk) parts.push(`Atención: con las dosis manuales el mínimo cae ${fmt(manualSafety.below, 1)} mg/dL por debajo del ideal.`);
  if (!parts.length) return "";
  return `<div class="glucosa-safety-note">${parts.join(" ")}</div>`;
}
...
function renderResult(state, controls) {
  ...
  const model = simulation.model;
  const band = simulation.optimizedPlan.idealBand || model.idealBand || fallbackIdealBand(model.config);
  ...
  const manualSafety = manualWithInsulin ? { hasHypoRisk: withInsMin.value < band.low, below: Math.max(0, band.low - withInsMin.value) } : null;
  const totalUnits = activeDoses.reduce((sum, dose) => sum + num(dose.units, 0), 0);
  const risk = withInsMin && withInsMin.value < band.low ? "danger" : withInsPeak && withInsPeak.value > band.high ? "warning" : "safe";
  ...
  return `
    <div class="glucosa-summary-grid">
      ...
      <div class="glucosa-kpi ${risk}"><span>Curva con dosis</span><strong>${withInsPeak ? fmt(withInsPeak.value) : "—"} / ${withInsMin ? fmt(withInsMin.value) : "—"}</strong><small>máx/mín · ideal ${fmt(band.low)}-${fmt(band.high)} · sin insulina ${fmt(noInsPeak.value)} mg/dL</small></div>
    </div>
    ${renderSafetyNote(simulation.optimizedPlan, manualSafety)}
    ...
    <div class="glucosa-summary-grid two">
      ...
      <div class="glucosa-kpi insulin"><span>Dosis activa</span><strong>${fmt(totalUnits, 1)} U</strong><small>Optimizada segura ${fmt(simulation.optimizedPlan.totalUnits, 1)} U · corrección ${fmt(simulation.optimizedPlan.correctionUnits, 1)} U · ISF ${fmt(simulation.warsaw.effISF, 1)}</small></div>
    </div>

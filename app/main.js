...
async function listPacksIntoUi() {
  ...
  root.innerHTML = remotePackFiles.length
    ? remotePackFiles.map((f, index) => `<div class="item pack-file-item" data-search="${escapeHtml([f.name, f.path].join(" "))}"><strong>${escapeHtml(f.name)}</strong><p class="qty-line">${escapeHtml(f.path)}</p><button data-action="install-remote-pack" data-index="${index}">Previsualizar e instalar</button></div>`).join("")

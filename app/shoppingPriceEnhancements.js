...
function ensureWeeklyTotal(root = document) {
  let totalNode = root.querySelector?.("[data-shopping-supermarket-total]");
  if (totalNode) return totalNode;
  const card = root.querySelector?.("[data-shopping-filter-root]") || root.querySelector?.(".supermarket-card");
  if (!card) return null;
  totalNode = document.createElement("div");
  totalNode.className = "item price-source-card";
  totalNode.dataset.shoppingSupermarketTotal = "true";
  const filterBar = card.querySelector(".shopping-filter-bar");
  if (filterBar) card.insertBefore(totalNode, filterBar);
  else card.prepend(totalNode);
  return totalNode;
}

function updateWeeklyTotal(root = document) {
  const totalNode = ensureWeeklyTotal(root);
  if (!totalNode) return;

...
function filterShoppingItems(items, filterId) { const filter = FILTERS.find(item => item.id === filterId) || FILTERS[0]; return items.filter(item => filter.statuses.includes(item.status)); }
function countForFilter(items, filter) { return items.filter(item => filter.statuses.includes(item.status)).length; }
function renderFilterButton(filter, activeFilter, items) {
  const active = filter.id === activeFilter; const count = countForFilter(items, filter);
...
function groupShoppingItems(state, items) {
  ...
  for (const item of items) {
    const family = familyMap.get(item.familyId) || "Otros"; const zone = supermarketZoneForFamily(family); const ingredient = ingredientMap.get(item.ingredientId); const fastPurchase = isTrustedPurchaseEnabled(ingredient);
    groups[zone] ||= { zone, items: [] }; groups[zone].items.push({ ...item, ingredient, family, zone, fastPurchase });
  }
  return groups;
}
...
function renderShoppingGroup(state, group) {
  ...
  return `<details class="shopping-zone compact-shopping-zone" open><summary><strong>${escapeHtml(group.zone)}</strong><span class="badge">${active}/${group.items.length}</span></summary><div class="list supermarket-list compact-supermarket-list">${group.items.map(item => renderIngredientCard(state, item.ingredient || {}, { mode: "shop", shoppingItem: item })).join("")}</div></details>`;
}

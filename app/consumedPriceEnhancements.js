...
function hydrateWeekPrices(root, state, dishes) {
...
function hydrateInstalledPackPrices(root, state) {
  root.querySelectorAll(".installed-pack-item").forEach(card => {
    ...
    if (!packId) return;
    const packDishes = (state.dishes || []).filter(dish => dish.packId === packId);
    const rows = [...card.querySelectorAll(".pack-installed-dish-list > li")];
    rows.forEach((row, index) => {
      const dish = packDishes[index];
      if (!dish) return;
      let node = row.querySelector("[data-installed-pack-dish-price]");
      if (!node) {
        node = document.createElement("small");
        node.className = "muted";
        node.dataset.installedPackDishPrice = dish.id;
...
export function hydrateConsumedPrices(root = document) {
  ...
  hydrateWeekPrices(root, state, dishes);
  hydrateInstalledPackPrices(root, state);
}
